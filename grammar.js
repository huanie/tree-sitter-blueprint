module.exports = grammar({
	...
	object: $ => seq($.type_name, optional(alias($.ident, 'id')), $.object_content),
	object_content: $ => seq('{', repeat(
	    choice(
		$.signal,
		$.property,
		$.extension,
	...
	child_internal: $ => seq('internal-child', alias($.ident, 'internal_child')),
	template: $ => seq(
	    'template',
	    $.type_name,
	...
	flags: $ => seq(
	    alias($.ident, 'first'),
	    repeat(seq('|', alias($.ident, 'rest')))
	),
	translated: $ => choice(
	    seq(
		'_', '(', alias($.quoted, 'string'), ')'
	    ),
	    seq('C_', '(', alias($.quoted, 'context'), alias($.quoted, 'string'), ')')
	),
	...
	closure_expression: $ => seq('$', alias($.ident, 'name'), '(', repeat(seq($.expression, ',')), ')'),
	cast_expression: $ => seq('as', '<', $.typename, '>'),
	menu: $ => seq('menu', optional(alias($.ident, 'id')), '{', repeat($.menu_child), '}'),
	menu_child: $ => choice(
	    $.menu_section,
	    $.menu_submenu,
	    $.menu_item_shorthand,
	...
	menu_section: $ => seq('section', optional(alias($.ident, 'id')), '{', repeat(choice($.menu_child, $.menu_attribute)), '}'),
	menu_submenu : $ => seq('submenu', optional(alias($.ident, 'id')), '{', repeat(choice($.menu_child, $.menu_attribute)), '}'),
	menu_item: $ => seq('item', '{', repeat($.menu_attribute), '}'),
	menu_attribute: $ => seq(alias($.ident, 'name'), ':', $.string_value, ';'),
	menu_item_shorthand: $ => seq('item', '(', $.string_value,
	    optional(
		seq(',',
		    optional(
			seq(
			    $.string_value, optional(
				seq(',', optional($.string_value)
				)
			    )
			)
		    )
